...
- **`dataagent_bench/`** — a solution pipeline for KDD Cup 2026 DataAgent-Bench (task.json + context/ → prediction.csv): deterministic context profiling, a code-executing solve agent, format validation/repair, self-consistency voting, cross-route verification, and a local eval harness. See `dataagent_bench/README.md`. Offline check: `python -m dataagent_bench.smoke_test`.
